...
class Simple_Dueling_DQN(nn.Module):
    def __init__(self, obs_space_shape, action_space_shape):
        """
        Dueling Network Architecture that estimates the value function & advantage function, aggregating them to predict the Q-value for discrete observation_space low dimensional environments
        obs_space_shape:int, env.observation_space.shape[0] = (for Cartpole = 4)
        action_space_shape:int, env.action_space.n = (for Cartpole = 2)
        """
        super(Simple_Dueling_DQN, self).__init__()
...
def main():
    ...
    qnet = Simple_Dueling_DQN(obs_space_shape, action_space_shape).to(device)
    target_qnet = Simple_Dueling_DQN(obs_space_shape, action_space_shape).to(device)

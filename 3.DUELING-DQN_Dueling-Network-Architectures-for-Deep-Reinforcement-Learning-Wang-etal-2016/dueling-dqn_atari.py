...
class Atari_Dueling_DQN(nn.Module):
    def __init__(self, obs_space_shape, action_space_shape):
        """
        Dueling Network Architecture that estimates the value function & advantage function, aggregating them to predict the Q-value for Atari Games
        obs_space_shape:int, env.observation_space.shape[0] 
        action_space_shape:int, env.action_space.n 
        """
        super(Atari_Dueling_DQN, self).__init__()
    ...
    def forward(self, obs):
        ...
        val_estimate = self.val(out)
        adv_estimate = self.adv(out)
        out = val_estimate + adv_estimate - adv_estimate.mean()
        return out
...
def main():
    ...
    target_qnet = Atari_Dueling_DQN(obs_space_shape, action_space_shape).to(device)
